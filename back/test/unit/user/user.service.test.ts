...
function createTestMocks() {
  ...
  const createChainableMock = (resolvedValue: any) => ({
    ...
    then: jest.fn().mockImplementation(callback => Promise.resolve(callback(resolvedValue))),
  ...
  const mockPasswordService = {
    hashPassword: jest.fn().mockResolvedValue('$argon2id$v=19$m=65536,t=3,p=4$hashedPassword123'),
...
describe('UserService', () => {
  ...
  beforeEach(async () => {
    ...
    jest.spyOn(argon2, 'hash').mockResolvedValue(passwordOperations.hashResults.hash);
  });
  ...
  describe('updateById', () => {
    ...
    it('should hash password when updating password', async () => {
      ...
      expect(passwordService.hashPassword).toHaveBeenCalledWith('NewPassword123!');
    });
  ...
  describe('removeById', () => {
    it('should delete user and cleanup related data', async () => {
      ...
      mockUserModel.findById.mockReturnValue(createSessionExecMock(validUsers[0]));
      ...
      mockUserModel.findByIdAndDelete.mockReturnValue(createSessionExecMock(validUsers[0]));
    ...
    it('should handle user not found during delete', async () => {
      ...
      await expect(userService.removeById(userId)).rejects.toThrow(NotFoundException);
      expect(mockSession.endSession).toHaveBeenCalled();
    });
  });
  ...
  describe('user stats and data retrieval', () => {
    it('should get user favorites', async () => {
      ...
      mockPlanModel.find.mockReturnValue({
        ...
        skip: jest.fn().mockReturnThis(),
        session: jest.fn().mockReturnThis(),
        then: jest.fn().mockImplementation(callback => Promise.resolve(callback(favorites))),
        catch: jest.fn().mockReturnThis(),
      });
