...
class PlanInfoSectionState extends State<PlanInfoSection> {
  ...
  void initState() {
    ...
    _favoritesCount = widget.plan.favorites.length;
    _loadCurrentUserAndAuthor();
  }

  Future<void> _loadCurrentUserAndAuthor() async {
    try {
      _currentUserId = await widget.viewModel.getCurrentUserId();
      _loadAuthorProfile();
    } catch (e) {
  ...
  Future<void> _toggleFavorite() async {
    ...
    try {
      if (_isFavorite) {
        await widget.viewModel.removeFromFavorites(widget.plan.id!);
        setState(() {
          _isFavorite = false;
          _favoritesCount--;
        });
        ...
        await widget.viewModel.addToFavorites(widget.plan.id!);
        setState(() {
          _isFavorite = true;
          _favoritesCount++;
        });
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          ...
          action: SnackBarAction(
            ...
            onPressed: () async {
              final userId = await widget.viewModel.getCurrentUserId();
              if (userId != null && mounted) {
                GoRouter.of(context).go(Routes.profil);
  ...
  Future<void> _loadAuthorProfile() async {
    ...
    try {
      final author = await widget.viewModel.getUserProfile(widget.plan.userId!);

      bool isFollowing = false;
      if (_currentUserId != null &&
          _currentUserId != widget.plan.userId &&
          author != null) {
        isFollowing = await widget.viewModel.isFollowing(author.id);
      }
  ...
  Future<void> _toggleFollow() async {
    ...
    try {
      final success = _isFollowing
          ? await widget.viewModel.unfollowUser(_authorProfile!.id)
          : await widget.viewModel.followUser(_authorProfile!.id);
