...
class MapPlanScreen extends StatefulWidget {
  final String planId;

  const MapPlanScreen({super.key, required this.planId});

  @override
  MapPlanScreenState createState() => MapPlanScreenState();
}

class MapPlanScreenState extends State<MapPlanScreen> {
  ...
  // Fetch steps from the server by planId
  void _fetchSteps() async {
    try {
      List<step_plan.Step> steps = await _stepService.fetchStepsByPlan(widget.planId);
      setState(() {
      ...
      if (_markers.isNotEmpty && mapController != null) {
        mapController!.zoomToBoundingBox(
          BoundingBox.fromGeoPoints(_markers),
          paddinInPixel: 50,
        );
      }
    } catch (e) {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _getCurrentLocation();
      _fetchSteps(); // Fetch steps after getting current location
    });
  }
